...
네이버 타일에는 구체적으로 **한글 라벨, 도로·건물 외곽선, 지하철 노선과 출구, 버스정류장을 포함한
대중교통 정보**가 들어 있습니다. 이들은 타일을 요청할 때 레이어 선택자를 명시해 모두 켠 것이며
(`bg.ol.ts.ar.lko`), 선택자를 붙이지 않으면 버스정류장·지하철출구 같은 항목이 빠진 타일이 옵니다.

**다크 테마도 지원합니다.** Home Assistant 가 다크 테마일 때는 네이버의 어두운 타일 계열(`dbasic`)을
사용하며, 한글 라벨·버스정류장·지하철출구·노선색은 라이트와 동일하게 들어 있습니다. 밝은 타일을
그대로 쓰고 싶으면 옵션에서 끌 수 있습니다.
...
**설정 → 기기 및 서비스 → Naver Map Change → 설정** 에서 바꿀 수 있습니다.
값을 바꾸면 통합이 다시 로드되고 타일 캐시가 비워집니다.

| 옵션 | 기본값 | 설명 |
|---|---|---|
| 제공자 | `naver` | 위 [지도 제공자](#지도-제공자-provider) 표 참조 |
| 인증키 / 타일 URL 템플릿 / 출처 표기 | 비어 있음 | 제공자가 요구할 때만 사용합니다. 인증키는 서버에만 보관되며 브라우저로 나가지 않습니다 |
| 제공자에 다크 타일이 있으면 사용 | 켜짐 | `naver` 에도 다크 타일 계열(`dbasic` 스타일)이 있으며, 한글 라벨·버스정류장·지하철출구가 라이트와 동일하게 들어 있습니다. Home Assistant 가 다크 테마일 때 자동으로 쓰입니다. 끄면 다크 테마에서도 라이트 타일을 사용합니다 |
| **고해상도 화면에서 선명한 타일 사용** | **켜짐** | 화면 픽셀 비율이 2 이상인 기기에서 512px `@2x` 타일을 받아옵니다. 지도가 선명해지지만 **데이터 사용량이 약 3배** 늘어납니다. `naver` 만 이 타일을 제공하며, 다른 제공자에서는 아무 영향이 없습니다 |
| 타일 캐시 크기 | 32MB | 서버 메모리에만 저장하며 디스크에는 아무것도 쓰지 않습니다. `@2x` 타일은 한 장당 약 3배이므로, 고해상도 화면을 쓰면 같은 용량에 담기는 타일 수가 약 1/3 로 줄어듭니다 |
...
- **벡터 지도 → 래스터 지도 하향.** Home Assistant 2026.9 의 기본 지도는 벡터 타일이지만 이 통합은
  래스터(이미지) 타일을 넣습니다. 그래서 **지도 라벨이 회전·줌에 따라 재배치되지 않고**, 제공자가 미리
  그려 놓은 위치에 고정됩니다. 이 한계는 래스터 타일의 성질이므로 없앨 수 없습니다. 그 대가로 한국
  지역의 한글 라벨·도로·건물·대중교통 품질이 크게 향상됩니다.
  **이 트레이드오프가 이 프로젝트가 존재하는 이유입니다.**
  - **선명도 문제는 v2.1.0 에서 완화됐습니다.** 고해상도(Retina) 화면에서 지도가 흐리게 보이던 문제는
    네이버가 제공하는 512px `@2x` 타일을 화면 픽셀 비율에 맞춰 받아오도록 하여 해결했습니다. 무손실
    PNG 타일을 사용하므로 글자·얇은 선의 압축 잡티도 없습니다.
    데이터 사용량이 약 3배 늘어나므로, 통합 옵션의 **"고해상도 화면에서 선명한 타일 사용"** 으로 끌 수
    있습니다(기본값 켜짐). 이 타일은 네이버만 제공하며, 다른 제공자에서는 이 옵션이 아무 영향을 주지
    않습니다.
